...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<EnumKYC>(translator.PushEnumKYC, translator.Get, translator.UpdateEnumKYC);
        ...
        int EnumKYC_TypeID = -1;
		int EnumKYC_EnumRef = -1;
        
        public void PushEnumKYC(RealStatePtr L, EnumKYC val)
        {
            if (EnumKYC_TypeID == -1)
            {
			    bool is_first;
                EnumKYC_TypeID = getTypeId(L, typeof(EnumKYC), out is_first);
				
				if (EnumKYC_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(EnumKYC));
				    EnumKYC_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, EnumKYC_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, EnumKYC_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for EnumKYC ,value="+val);
            }
			
			LuaAPI.lua_getref(L, EnumKYC_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out EnumKYC val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != EnumKYC_TypeID)
				{
				    throw new Exception("invalid userdata for EnumKYC");
				}
                ...
                {
                    throw new Exception("unpack fail for EnumKYC");
                }
				val = (EnumKYC)e;
            ...
            {
                val = (EnumKYC)objectCasters.GetCaster(typeof(EnumKYC))(L, index, null);
            }
        }
		
        public void UpdateEnumKYC(RealStatePtr L, int index, EnumKYC val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for EnumKYC ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(EnumKYC[]))
			{
			    EnumKYC[] array = obj as EnumKYC[];
				translator.PushEnumKYC(L, array[index]);
				return true;
			}
